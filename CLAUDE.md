# Claude Code Project Instructions — rama-unix-kotlin

## Project Overview

This is **rama-unix-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`rama-unix`](https://crates.io/crates/rama-unix).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/rama-unix/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/rama-unix>
package io.github.kotlinmania.ramaunix
```
